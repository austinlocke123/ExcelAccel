...
public static class DiscoveryCommandCatalog
{
    private static readonly IReadOnlyList<CommandDescriptor> Commands = new[]
    {
        ...
        new CommandDescriptor("favorite.add", 1, "Add Favorite", CommandImpact.Low,
            new[] { "user_profile_favorites" }, true, RibbonRoutes.For("favorite.add"), "CAP-FAV-001",
            CommandContextRequirement.Application, PreviewPolicy.None, UndoPolicy.None,
            new[] { "AC-FAV-001", "AC-FAV-003" }, "Discovery",
            "Add the selected command and fixed arguments to the local profile.", shortcutLabel: RibbonRoutes.For("favorite.add")),
        new CommandDescriptor("favorite.remove", 1, "Remove Favorite", CommandImpact.Low,
            new[] { "user_profile_favorites" }, true, RibbonRoutes.For("favorite.remove"), "CAP-FAV-001",
            CommandContextRequirement.Application, PreviewPolicy.None, UndoPolicy.None,
            new[] { "AC-FAV-001", "AC-FAV-003" }, "Discovery",
            "Remove the selected favorite from the local profile.", shortcutLabel: RibbonRoutes.For("favorite.remove")),
        new CommandDescriptor("favorite.invoke", 1, "Invoke Favorite", CommandImpact.ReadOnly,
            new string[0], true, RibbonRoutes.For("favorite.invoke"), "CAP-FAV-001",
            CommandContextRequirement.Application, PreviewPolicy.None, UndoPolicy.None,
            new[] { "AC-CMD-002", "AC-FAV-002", "AC-FAV-003", "AC-FAV-004" }, "Discovery",
            "Resolve the current command contract and route through its normal lifecycle.", shortcutLabel: RibbonRoutes.For("favorite.invoke"),
