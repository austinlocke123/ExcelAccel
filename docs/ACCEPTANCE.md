...
| AC-FMT-032 | `formula.units.to_basis_points` multiplies by 10,000, applies a basis-point format, and carries the same impact tier, preview, and undo as the other unit transforms. **Fully met (WP-F-10): value and format move on one batch receipt, so a single undo reverses both.** |
